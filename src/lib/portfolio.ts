...
export async function getPortfolioSummary(): Promise<PortfolioSummary> {
  try {
    const [allDeposits, allPortfolioSnaps, activeAssets, allSnapshots] = await Promise.all([
    ...
    const assetsWithValues: AssetWithLatestSnapshot[] = activeAssets.map((asset) => {
      ...
      return { ...asset, latestValue, totalDeposited: assetDeposited, pnl, returnPct }
    })

    const currentValue = assetsWithValues.reduce((s, a) => s + a.latestValue, 0)
