...
import { formatDate, formatZAR, formatPct } from '@/lib/formatting'
    ...
    <div className="p-4 md:p-6">
      ...
      {snapshots.length === 0 ? (
        <div className="rounded-lg border bg-card p-8 text-center">
          ...
          <Link href="/dashboard/snapshots/new" className="mt-2 inline-block text-sm text-primary hover:underline">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b">
                  ...
                  <th className="px-4 py-2.5 text-right text-xs font-medium uppercase tracking-wide text-muted-foreground">Total value</th>
                ...
                {snapshots.map((snap, idx) => {
                  ...
                  return (
                    <tr key={snap.id} className="border-b last:border-b-0 hover:bg-muted/30 transition-colors">
                      <td className="px-4 py-3">{formatDate(snap.snapshotAt)}</td>
                      <td className="px-4 py-3 text-right font-mono font-medium">{formatZAR(totalValue)}</td>
                      <td className="px-4 py-3 text-right font-mono text-muted-foreground">{formatZAR(totalDeposited)}</td>
                      <td className={cn('px-4 py-3 text-right font-mono', pnl >= 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-red-500 dark:text-red-400')}>
                        {pnl >= 0 ? '+' : ''}{formatZAR(pnl)}
                      </td>
                      <td className="px-4 py-3 text-right">
                        ...
                        {weekChange !== null && (
                          <div className={cn('text-xs mt-1', weekChange >= 0 ? 'text-emerald-600' : 'text-red-500')}>
                            {weekChange >= 0 ? '+' : ''}{formatZAR(weekChange)}
                          </div>
                        )}
                      </td>
                    </tr>
