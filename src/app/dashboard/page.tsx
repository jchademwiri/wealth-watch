...
export default async function DashboardPage() {
  ...
  const daysSinceSnap = latestSnap
    ? Math.floor((Date.now() - new Date(latestSnap.snapshotAt).getTime()) / 86400000)
    : null
    ...
    <div className="space-y-6 p-4 md:p-6">
      ...
      <div className="rounded-lg border bg-card p-4">
        <div className="mb-3 flex items-center justify-between">
          <h3 className="text-sm font-medium text-muted-foreground">Recent deposits</h3>
          <Link href="/dashboard/deposits" className="text-xs text-primary hover:underline">
            View all
          </Link>
        </div>
          ...
          {allDeposits.slice(0, 5).map(d => (
            <div key={d.id} className="flex items-center justify-between text-sm">
              <div className="flex items-center gap-2">
                ...
                />
                <span className="text-muted-foreground">{(d as any).asset?.name}</span>
                <span className="text-xs text-muted-foreground">{formatDate(d.depositedAt)}</span>
              </div>
              <span className="font-mono font-medium">{formatZAR(parseFloat(d.amount))}</span>
            </div>
          ))}
