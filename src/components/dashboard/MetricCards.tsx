import { TrendingUp, TrendingDown, Minus, Wallet, PiggyBank, BarChart3 } from 'lucide-react'
import { formatZAR, formatPct, pnlColor, pnlBadgeClass } from '@/lib/formatting'
...
export function MetricCards({
  ...
  const TrendIcon = returnPct > 1 ? TrendingUp : returnPct < -1 ? TrendingDown : Minus
...
function MetricCard({ label, value, valueRaw, icon, sub, highlight }: MetricCardProps) {
  ...
  return (
    <div className="rounded-lg bg-muted/50 p-4">
      ...
      {highlight && valueRaw !== undefined ? (
        <span className={cn('mt-1 inline-block rounded px-1.5 py-0.5 text-xs font-medium', pnlBadgeClass(valueRaw))}>
          {sub}
        </span>
      ) : (
